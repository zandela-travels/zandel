...
const PGCompany = ({vehicleData}) => {
  ...
  useEffect(() => {
    axios.get(`${url}/details/${companyId}`)
      .then((response) => {
        setCompVehicle(response.data.vehicles);
        setPaymentForm(prevForm => ({
          ...prevForm,
          vehicleType: response.data.VehicleRegNo
        }));
      })
      .catch((error) => {
  ...
  const Reg = vehicleData.VehicleRegNo;
  const price = vehicleData['Price/Km'];
  ...
  const handleSubmit = (e) => {
    ...
    if (!paymentForm.Time) {
      ...
      axios.post(`${url}/company/payment/${companyId}`, paymentForm)
        .then(response => {
      ...
      <div className='pay-form'>
        ...
        <p className='p-head'>Instructions</p>
          ...
          <ul className='form-points'>
            ...
            <li>Bookings can only be cancelled prior to 2 days before the trip by contacting us on <span>0781799999</span>.</li>
            ...
            <li>For any inquiry call us  <span>0781799999</span></li>
          </ul>on
          </div>  
